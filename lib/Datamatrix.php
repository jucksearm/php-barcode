...
class Datamatrix
{
	...
	*/
	 ...
	 * @protected
	 */
	protected $last_enc = self::ENC_ASCII;
	 ...
	 * @protected
	 */
	protected $chset_id = array(self::ENC_C40 => 'C40', self::ENC_TXT => 'TXT', self::ENC_X12 =>'X12');
...
// -----------------------------------------------------------------------------
	...
	protected function lookAheadTest($data, $pos, $mode) {
		...
		while (true) {
			...
			// STEP O
			if ($this->isCharMode($chr, self::ENC_X12) OR $this->isCharMode($chr, self::ENC_C40)) {
		...
		while ($pos < $data_length) {
			...
			switch ($enc) {
				case self::ENC_ASCII: { // STEP B. While in ASCII encodation
					if (($data_length > 1) AND ($pos < ($data_length - 1)) AND ($this->isCharMode(ord($data[$pos]), self::ENC_ASCII_NUM) AND $this->isCharMode(ord($data[$pos + 1]), self::ENC_ASCII_NUM))) {
				...
				case self::ENC_X12 : { // ANSI X12
					...
					do {
						...
						if (isset($charset[$chr])) {
							...
							} elseif (($enc == self::ENC_C40) AND isset($this->chset['S3C'][$chr])) {
								$temp_cw[] = 2; // shift 3
								$shiftset = $this->chset['S3C'];
							} elseif (($enc == self::ENC_TXT) AND isset($this->chset['S3T'][$chr])) {
								$temp_cw[] = 2; // shift 3
				...
				case self::ENC_EDF: { // F. While in EDIFACT (EDF) encodation
					...
					do {
						...
						if (($field_length == 4) OR ($epos == $data_length) OR !$this->isCharMode($chr, self::ENC_EDF)) {
